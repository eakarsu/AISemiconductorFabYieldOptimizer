// AI Yield prediction by process parameters
// Real-time model predicts yield, alerts to process drift
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AISemiconductorFabYieldOptimizer'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISemiconductorFabYieldOptimizer. Focus area: Yield prediction by process parameters. ${`Real-time model predicts yield, alerts to process drift`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Yield prediction by process parameters.\n${`Real-time model predicts yield, alerts to process drift`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'yield-prediction', context, parsed);
    res.json({ feature: 'yield-prediction', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[yield-prediction]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['yield-prediction']);
