...
router.get('/', async (req, res) => {
  try {
    ...
    const countQ = await pool.query('SELECT COUNT(*) FROM equipment_matching');
    const total = parseInt(countQ.rows[0].count);
    const dataQ = await pool.query('SELECT * FROM equipment_matching ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
